...
public class ProductTester {
	...
	public static void addCDToInventory(Product[] products, Scanner in) {
		...
		for(int i = 0; i<products.length; i++) {
    		...
    		products[i] = new CD(tempNumber, tempName, tempQty, tempPrice, tempArtist, tempNumOfSongs, tempLabel);
        }//endfor
	}
	...
	public static void addDVDToInventory(Product[] products, Scanner in) {
		...
		for(int i = 0; i<products.length; i++) {
    		...
    		tempNumber = in.nextInt();
    		//create a product object and store it in the products array
    		products[i] = new DVD(tempNumber, tempName, tempQty, tempPrice, tempLenght, tempAgeRating, tempFilmStudio);
        }//endfor
	}
